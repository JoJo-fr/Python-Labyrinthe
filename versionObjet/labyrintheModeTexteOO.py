...
class LabyrintheModeTexte(object):
    ...
    def afficheLabyrinthe(self, message = "", sauts = 0):
        ...
        for i in range(1, self.labyrinthe.getNbJoueurs() + 1):
        	pcouleur('Joueur ' + str(i) + ' ' + str(self.labyrinthe.nbTresorsRestants(
        		i)) + ' ', i)
        print()
        print("C'est au tour du ", end = '')
        pcouleur('Joueur ' + str(self.labyrinthe.getJoueurCourant()) +
        	" de jouer", self.labyrinthe.getJoueurCourant())
        print()
        ...
        for j in range(plateau.getNbColonnes()):
        	self.afficheCarte(plateau.getVal(i, j), self.labyrinthe.getJoueurCourant(),
        		tresor)
        if i % 2 == 0:
        	print(' ', sep = '', end = '')
    else :
    ...
    def main(self):
        ...
        while not fini:
            ...
            while (not finOrdre):
                ...
                self.afficheLabyrinthe("La carte a été insérée en " + ordre[0].upper() +
                	" " + ordre[1])
                finOrdre = True
